	...
	function buildFilterBar(target, rows){
		...
		$.map(opts.pivot.filters, function(field){
			...
			function handler2(){
				...
				$(this).combobox('setText', values.length ? (values.length < 3 ? values : 'multiple items') : '');
			}
		});
	...
	function layout(target){
		...
		if (!state.layoutDialog){
			var content = '<div class="easyui-layout" fit="true">' +
					'<div region="west" split="true" class="pg-fields" title="' + opts.i18n.fields + '" style="width:180px"></div>' +
	...
	$.fn.pivotgrid.defaults = $.extend({}, $.fn.treegrid.defaults, {
		...
		operators:{
			...
			avg: function(rows, field){
				...
				// console.log(v,rows)
				return rows.length != 0?(v/rows.length).toFixed(0):'-'
			},
